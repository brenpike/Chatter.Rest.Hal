...
1. Strip URI scheme prefix: if the string begins with a URI scheme pattern matching `^[a-z][a-z0-9+.-]*://` (case-insensitive), remove the scheme and `://` (e.g., `https://` and `HTTPS://` are both removed, leaving `api.example.com/orders`). Relative hrefs (starting with `/` or any non-scheme character) are unaffected. The authority (host + optional port) is retained and processed by subsequent steps.
