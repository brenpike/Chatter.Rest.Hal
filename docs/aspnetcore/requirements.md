...
**REQ-14:** `Template<TController>(Expression<Action<TController>>)` and `Template<TController>(Expression<Func<TController, Task<IActionResult>>>)` extension methods on `IHalLinkBuilder` resolve the route name from action attributes using the same logic as REQ-13, then delegate to `Template(routeName)`. Same namespace as REQ-13.
